...
public class ClusterManager extends Application {
	...
	public void start(Stage primaryStage) {
		try {
			...
			primaryStage.setScene(GraphicUserInterface.getSecondScene());
			primaryStage.show();
			
			
			GraphicUserInterface.btn.setOnAction(new EventHandler<ActionEvent>() {			
				@Override
				public void handle(ActionEvent arg0) {
					String ip = GraphicUserInterface.ipTextField.getText();
					String port = GraphicUserInterface.portField.getText();
					if(clusterManager.connectToClusterServer(port, ip) == true){
						primaryStage.setScene(GraphicUserInterface.getSecondScene());
					}
		...
		try {
			...
			if(Integer.parseInt(json.get("function_code").toString()) == 0)
				if (this.checkIfIamClusterBoss(json) == false){
					GraphicUserInterface.actiontarget.setText("Clusterul este deja in functiune!!!");
					return false;
				}
				else{
					return true;
				...
				GraphicUserInterface.actiontarget.setText(e.getMessage());
		} catch (UnknownHostException e) {
			GraphicUserInterface.actiontarget.setText("Unknown host exception:" + e.getMessage());
		} catch (IOException e) {
			GraphicUserInterface.actiontarget.setText(e.getMessage());
		} catch (NumberFormatException e) {
			GraphicUserInterface.actiontarget.setText("[PORT]" + "Contine doar cifre!");
		}
		return false;
	}
