...
Json::Value Solutie::solutieToJson() {
    Json::Value json;
    json["dimensiune"] =  this->dimensine;
    for(int i = 0; i < this->dimensine; i++){
        json[std::to_string(i)] = this->getEncodareLocatie(i);
    }
    return json;
}

void Solutie::jsonToSolutie(Json::Value json) {
    this->dimensine = json["dimensiune"].asInt();
    for(int i = 0; i < this->dimensine; i++){
        this->setInfo(i, json[std::to_string(i)].asInt());
    }
}

int Solutie::getNumarOrase() {
    return  this->dimensine;
}

void Solutie::setNumarOrase(int nr) {
   this->dimensine = nr;
    this->solutie.clear();
}

void Solutie::copyTraseu(Solutie sol) {
    for(int i = 0; i < this->dimensine; i++)
        this->addLocation(sol.getEncodareLocatie(i));
}
